...
const GoodsList: React.FC<{ props: any }> = (props) => {
  ...
  const { eventId, eventTitle } = query
  const { cateData, getExchangeLabel } = useModel('dataModel');
  ...
  const getEventGoods = async (params: any, page = 1, size = 24) => {
    setSpinning(true)
    ...
    if (listData && listData.length > 0) {
      if (brandId) {
        ...
        if (brandId !== newSelected.join('')) {
          ...
          getEventGoods(setParams('brand_id', newSelected.join('')))
        }
      }
    } else if (brandId) {
      setBrandId('')
      getEventGoods(setParams('brand_id', null))
    }
  }
  ...
  useEffect(() => {
    getEventGoods({ event_id: eventId })
  }, []);
  ...
  const clickSearch = () => {
    ...
    if (value) {
      ...
      getEventGoods(setParams('title', value))
    }
  }
  ...
  const checkShowPrice = () => {
    if (goodsListData && 'items' in goodsListData && goodsListData.items) {
      ...
      if (items.length > 0) {
        return items[0]?.events?.show_price || false
      }
      return false
    }
    return false
  ...
  const renderLeftSearchOption = () => {
    return (
      <div className="col-lg-4 mb-50">
          ...
          <div className="text-center mt-20">
            <a
              ...
              onClick={() => {
                ...
                getEventGoods(setParams('priceRange', priceRange))
              }}>{formatMessage({
        ...
        {cateData && cateData.length > 0 && <div className="widget">
          <h5 className="title">{formatMessage({ id: 'project.goods.cate', defaultMessage: '拍卖品类型' })}</h5>
          <Checkbox.Group
            ...
            onChange={(value) => {
              ...
              getEventGoods(setParams('category_id', value.join(',')))
            }}
          >
            <Row>
              {cateData.map((item: any) => (
                <Col span={24} key={item.id}>
                  <Checkbox value={`${item.id}`}>{item.category_name}</Checkbox>
                </Col>
              ))}
            </Row>
          </Checkbox.Group>
        ...
        {brandData && brandData.length > 0 && <div className="widget">
          ...
          <Checkbox.Group
            ...
            onChange={(value) => {
              ...
              getEventGoods(setParams('brand_id', value.join(',')))
            }}
          >
            <Row>
        ...
        <div className="widget">
          ...
          <Checkbox.Group
            style={{ width: '100%', maxHeight: '180px', overflowY: 'auto' }}
            value={caseState.split(',')}
            onChange={(value) => {
              setCaseState(value.join(','))
              getEventGoods(setParams('case_state', value.join(',')))
            }}
          >
            <Row>
    ...
    return (
      <div id='pageTop' className="product-header mb-40" style={{ backgroundColor: 'white' }}>
        <div style={{ width: '100%', marginTop: '20px' }}>
          <h5 className="title mb-3" style={{ textAlign: 'center' }}>{eventTitle}</h5>
        </div>
        <div className="product-header-item">
          ...
          <Select
            ...
            onChange={(value) => {
              setOrderBy(value)
              getEventGoods(setParams('order_by', value))
            }}>
            ...
            {checkShowPrice() && <Select.Option value="start_price">{formatMessage({ id: 'project.goods.startPrice', defaultMessage: '起拍价' })}</Select.Option>}
          </Select>
        </div>
        <div className="product-header-item">
          ...
          <Select
            ...
            onChange={(value) => {
              ...
              getEventGoods(setParams('event_status', value))
            }}>
            <Select.Option value=''>{formatMessage({ id: 'project.goods.all', defaultMessage: '全部' })}</Select.Option>
        ...
        {orderType === 'asc' ?
          <ArrowUpOutlined title='升序' onClick={() => {
            setOrderType('desc')
            getEventGoods(setParams('order_type', 'desc'))
          }} /> :
          <ArrowDownOutlined title='降序' onClick={() => {
            setOrderType('asc')
            getEventGoods(setParams('order_type', 'asc'))
          }} />
        }
    ...
    <>
      ...
      <div className="featured-auction-section padding-bottom mt--240 mt-lg--440 pos-rel">
        <div className="container2" style={{ marginTop: '120px' }}>
          <PageBreadcrumb routes={[{ name: formatMessage({ id: 'project.goods.pageBreadGoods', defaultMessage: '活动商品列表' }), path: null }]} />
          ...
          {goodsListData &&
            <div style={{ width: '100%', textAlign: 'center', marginTop: '40px' }}>
              <Pagination
                ...
                onChange={(page, size) => {
                  // setPageNo(page)
                  getEventGoods(setParams('', null), page, size)
